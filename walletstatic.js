 /* ./static/script.js */ (function(window) {
    var ethers = window.ethers;
    var defaultUrl = "http://localhost:8080/dapp.html";
    var walletUrl = "http://localhost:8080/dapp.html";
    ...
    var navigation = function() {
        ...
        document.body.ondragover = function(event) {
            ...
            if (!topController.ondrag && !topController.ondrop) {
                return
            }
        ...
        document.body.ondragenter = function() {
            ...
            if (enteredIndex >= 0) {
                ethersLog("This should not happen", entered)
            }
        ...
        defineProperty(navigation, "clear", function(purge) {
            if (controllers.length > 1) {
                var topController = controllers[controllers.length - 1];
                if (purge) {
                    if (topController.oncancel) {
                        topController.oncancel()
                    }
        ...
        defineProperty(navigation, "pushNotice", function(title, blurb) {
            ...
            function complete() {
                ...
                if (controller.ondone) {
                    controller.ondone.call(controller)
                }
            }
            controller.oncancel = function() {
                complete()
            };
            timer = setTimeout(complete, 5e3);
            return controller
        });
        ...
        defineProperty(navigation, "addAccount", function(address, json, nickname) {
            ...
            var div = nodes[0].cloneNode(true);
            forEach(div, ".populate-nickname", function(el) {
                el.value = nickname
            });
    ...
    var network = function() {
        ...
        if (!network) {
            ...
            // change network here ; where to change network
        }
        return network
    }();
    defineProperty(ethers, "versionHash", "0x1c20ffae6e4410fcf1e85ec093cbac5374e4da2e8a83ab5379c0588a4a2be21d");
    var provider = null;
    if (network === "test") {
        (function() {
            var query = {};
            ...
            var host = "localhost";
            if (query.host && query.host.match(/^[A-Za-z0-9-.]+$/)) {
                host = query.host
            }
            var path = "/";
    ...
    var wallet = function() {
        ...
        var gasLimitTiers = [21e3, 75e3, 15e4, 25e4, 5e5, 75e4, 15e5];

        function getTransactionInfo(tx) {
            return Promise.all([tx.to ? provider.getCode(tx.to) : Promise.resolve("INVALID"), provider.estimateGas(tx), provider.getTransactionCount(tx.from), getGasPrices()]).then(function(result) {
                var gasLimit = 21e3;
                var transfer = true;
                if (result[0] !== "0x") {
                    transfer = false;
        ...
        function Account(json) {
            ...
            defineLookup(this, "locked", function() {
                return wallet === null
            });
                        ...
                        return new Promise(function(resolve, reject) {
                            ...
                            controller.onbutton = function(el) {
                                ...
                                wallet.sendTransaction(tx).then(function(tx) {
                                    ...
                                    var controller = navigation.pushNotice("Transaction Sent", "Your transaction has successfully been sent to the Ethereum network.");
            ...
            function showMessage(messageData) {
                ...
                for (var i = 0; i < messageData.length; i++) {
                    var c = messageData[i];
                    dataString += hexChars[c >> 4] + hexChars[c & 15];
                    if (i % 8 === 7) {
                        dataString += " "
                    }
                    if ((c < 32 || c >= 127) && c !== 13) {
            ...
            var updateBalance = function(balance) {
                if (balance.toString() === store.get("balance")) {
                    return
                }
            ...
            return new Promise(function(resolve, reject) {
                ...
                controller.ondrop = function(content) {
                    if (content === self.json) {
                        ...
                        self.emit("didRemove");
                        var controller = navigation.pushNotice("Account Removed", 'The "' + self.nickname + '" has been removed from this browser');
                        controller.ondone = function() {
                            resolve()
                        }
                    } else {
        ...
        function MetaMaskAccount(web3) {
            if (!(this instanceof MetaMaskAccount)) {
                throw new Error("missing new")
            }
            ...
            function getWallet(text) {
                ...
                if (text.split(/ /g).length == 12) {
                    if (ethers.HDNode.isValidMnemonic(text)) {
                        return {
                            generate: function() {
                                return ethers.Wallet.fromMnemonic(text)
                            },
                            method: "mnemonic"
                        }
                    }
            ...
            return new Promise(function(resolve, reject) {
                ...
                controller.onbutton = function(el) {
                    ...
                    if (name === "create") {
                        ...
                        var controller = navigation.push("import", {
                            focus: false
                        });
                        controller.enabled = false;
        ...
        function load(url) {
            ...
            function go() {
                if (currentApplication) {
                    currentApplication.iframe.remove()
                }
            ...
            if (origin.substring(0, 5) === "https" || insecureUntil > (new Date).getTime()) {
                ...
                return new Promise(function(resolve, reject) {
                    ...
                    controller.onbutton = function() {
                        go();
                        navigation.clear();
                        settings.set("insecureUntil", String((new Date).getTime() + 30 * 60 * 1e3));
                        resolve(url)
                    };
                    controller.oncancel = function() {
                        reject(new Error("cancelled"))
        ...
        window.addEventListener("message", function(event) {
            ...
            switch (data.action) {
                case "ready":
                    ...
                    send({
                        action: "ready",
                        account: wallet.currentAccount ? wallet.currentAccount.address : null,
                        network: network
                    });
                    break;
                case "getAccount":
